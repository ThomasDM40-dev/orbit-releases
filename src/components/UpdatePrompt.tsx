...
export default function UpdatePrompt() {
  ...
  const updateAll = async () => {
    ...
    if (ytdlp) {
      setToolMsg(t('🔧 Mise à jour de yt-dlp…'));
      const res = await electron.updateYtdlp?.().catch(() => null);
      setToolMsg(res?.success ? t('✓ yt-dlp à jour') : '⚠ yt-dlp : ' + (res?.message || t('échec')));
      setYtdlp(null);
  ...
  if (appUpdate) items.push({ icon: '🚀', name: `Orbit ${appUpdate.version ? 'v' + appUpdate.version : ''}`, detail: t('Nouvelle version de l\'application') });
  if (ytdlp) items.push({ icon: '🌐', name: 'yt-dlp', detail: `${ytdlp.current} → ${ytdlp.latest || t('dernière')}` });
      ...
      <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/70 backdrop-blur-md p-4">
        <motion.div initial={{ scale: 0.94, opacity: 0, y: 10 }} animate={{ scale: 1, opacity: 1, y: 0 }} exit={{ scale: 0.94, opacity: 0 }} transition={{ duration: 0.25, ease: [0.16, 1, 0.3, 1] }}
          ...
          <div className="px-6 py-3 space-y-2">
            {items.length === 0 && phase !== 'ask' && <p className="text-center text-sm text-gray-400 py-2">{toolMsg || t('Traitement…')}</p>}
            {items.map((it, i) => (
          ...
          <div className="px-6 pb-6 pt-2 flex gap-2">
            {phase === 'ready' ? (
              <button onClick={install} className="flex-1 py-2.5 rounded-xl font-bold text-sm text-white" style={{ background: 'linear-gradient(135deg,#22c55e,#16a34a)' }}>🔄 {t("Redémarrer & installer")}</button>
            ) : phase === 'working' ? (
              <button disabled className="flex-1 py-2.5 rounded-xl font-bold text-sm text-white opacity-70" style={{ background: 'var(--accent,#ec4899)' }}>⏳ {t("En cours…")}</button>
            ) : (
              <>
                <button onClick={later} className="px-4 py-2.5 rounded-xl text-sm bg-white/5 border border-white/10 text-gray-300 hover:bg-white/10">{t("Plus tard")}</button>
                <button onClick={updateAll} className="flex-1 py-2.5 rounded-xl font-bold text-sm text-white" style={{ background: 'linear-gradient(135deg, color-mix(in srgb, var(--accent,#ec4899) 90%, white) , var(--accent,#ec4899))', boxShadow: '0 6px 20px color-mix(in srgb, var(--accent,#ec4899) 40%, transparent)' }}>⬆️ {t("Mettre à jour")}</button>
              </>
            )}
          </div>
          {phase === 'ready' && <button onClick={() => setOpen(false)} className="w-full pb-4 text-[11px] text-gray-500 hover:text-gray-300">{t("Installer au prochain démarrage")}</button>}
        </motion.div>
      </div>
    </AnimatePresence>
  );
