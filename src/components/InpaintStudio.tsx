...
export default function InpaintStudio() {
  ...
  const loadImage = useCallback((path: string) => {
    ...
    probe.onerror = () => setError(t('Impossible de charger cette image.'));
    probe.src = url;
  }, [resetMask]);
  ...
  const detectObjects = async () => {
    if (!img || processing) return;
    setError(null); setProcessing(true); setStage(t('Détection des objets…'));
    ...
    if (res?.ok) {
      ...
      if (!res.detections?.length) showToast(t('Aucun objet détecté.'));
      else showToast(t('{n} objet(s) détecté(s) — clique pour sélectionner.', { n: res.detections.length }));
    } else setError(res?.error || t('Détection échouée.'));
  };
  ...
  const run = async () => {
    ...
    if (!maskPng) { setError(t('Peins d\'abord la zone avec le pinceau.')); return; }
    const p = prompt.trim();
    setProcessing(true); setError(null); setStage(p ? t('Génération…') : t('Préparation…'));
    ...
    if (res?.ok) {
      ...
      showToast(p ? t('Zone générée ✓') : t('Objet supprimé ✓'));
    } else { setError(res?.error || t('Échec.')); }
  };
  ...
  return (
    <div className="h-full flex flex-col text-gray-300 overflow-hidden">
      ...
      <div className="flex items-center justify-between px-6 py-3 border-b border-white/5">
        ...
        {img && <button onClick={handleBrowse} className="px-3 py-1.5 rounded-xl text-xs font-medium bg-white/5 border border-white/10 hover:bg-white/10 flex items-center gap-1.5"><ImagePlus className="w-3.5 h-3.5" /> {t("Nouvelle image")}</button>}
      </div>
      ...
      <div className="flex-1 overflow-hidden flex">
        ...
        <div className="w-[300px] shrink-0 border-r border-white/5 flex flex-col p-4 gap-4 overflow-y-auto">
          ...
          <div>
            <label className="text-[11px] font-semibold text-gray-400 uppercase tracking-wider flex items-center gap-1.5"><Wand2 className="w-3 h-3 text-fuchsia-400" /> Prompt <span className="text-gray-600 normal-case font-normal tracking-normal">{t("(optionnel)")}</span></label>
            <textarea value={prompt} onChange={e => setPrompt(e.target.value)} rows={3}
              placeholder={t("Vide = effacer l'objet.\nSinon, décris quoi mettre à la place : « a leather backpack », « a tree »…")}
            ...
            <div className="mt-1.5 flex items-center gap-1.5 text-[10px]">
              <span className={`px-2 py-0.5 rounded-full border ${!prompt.trim() ? 'bg-rose-500/15 border-rose-500/40 text-rose-200' : 'bg-white/5 border-white/10 text-gray-500'}`}>{t("🧽 Effacer")}</span>
              <span className={`px-2 py-0.5 rounded-full border ${prompt.trim() ? 'bg-fuchsia-500/15 border-fuchsia-500/40 text-fuchsia-200' : 'bg-white/5 border-white/10 text-gray-500'}`}>{t("✨ Générer / Remplacer")}</span>
            </div>
            {prompt.trim() && <p className="text-[10px] text-gray-500 mt-1.5">{t("Inpainting Stable Diffusion")} <span className="text-gray-400">{t("local")}</span> {t(": l'IA analyse la photo autour de la zone et fond le rendu dans la scène (plus d'image aléatoire).")}</p>}
          </div>

          {/* Outils : sélection IA + pinceau + gomme */}
          <div>
            <label className="text-[11px] font-semibold text-gray-400 uppercase tracking-wider">{t("Outil")}</label>
            <button onClick={() => setTool('sam')} className={`mt-1.5 w-full px-3 py-2 rounded-xl text-sm font-semibold border flex items-center justify-center gap-1.5 transition-all ${tool === 'sam' ? 'bg-fuchsia-500/25 text-fuchsia-100 border-fuchsia-500/50' : 'bg-white/5 text-gray-300 border-white/10 hover:bg-white/10'}`}><MousePointerClick className="w-4 h-4" /> {t("Sélection IA (clic sur l'objet)")}</button>
            <div className="grid grid-cols-2 gap-2 mt-2">
              <button onClick={() => setTool('brush')} className={`px-3 py-2 rounded-xl text-sm font-medium border flex items-center justify-center gap-1.5 transition-all ${tool === 'brush' ? 'bg-rose-500/25 text-rose-100 border-rose-500/50' : 'bg-white/5 text-gray-400 border-white/10 hover:bg-white/10'}`}><Brush className="w-4 h-4" /> {t("Pinceau")}</button>
              <button onClick={() => setTool('eraser')} className={`px-3 py-2 rounded-xl text-sm font-medium border flex items-center justify-center gap-1.5 transition-all ${tool === 'eraser' ? 'bg-rose-500/25 text-rose-100 border-rose-500/50' : 'bg-white/5 text-gray-400 border-white/10 hover:bg-white/10'}`}><Eraser className="w-4 h-4" /> {t("Gomme")}</button>
            </div>
            {tool === 'sam' && <p className="text-[10px] text-gray-500 mt-1.5">{t("Clique sur un objet → masque auto. Re-clique pour agrandir.")} <span className="text-gray-400">{t("Maj/clic-droit")}</span> {t("= retirer une partie. (1ʳᵉ fois : ~40 Mo)")}</p>}
            <button onClick={detectObjects} disabled={!img || processing} className="mt-2 w-full px-3 py-2 rounded-xl text-xs font-medium bg-white/5 border border-white/10 hover:bg-white/10 disabled:opacity-40 flex items-center justify-center gap-1.5"><Scan className="w-3.5 h-3.5" /> {t("Détecter les objets")}</button>
            {detections.length > 0 && (
              <label className="flex items-center gap-2 text-[11px] text-gray-400 mt-1.5 cursor-pointer">
                ...
                {t("Afficher les {n} objets détectés", { n: detections.length })}
              </label>
            )}
          </div>
          ...
          <div className={tool === 'sam' ? 'opacity-40 pointer-events-none' : ''}>
            <div className="flex justify-between mb-1.5"><label className="text-[11px] font-semibold text-gray-400 uppercase tracking-wider">{t("Taille du pinceau")}</label><span className="text-[11px] font-mono text-gray-200">{brush}px</span></div>
          ...
          <div className="flex gap-2">
            <button onClick={clearMask} disabled={!hasMask} className="flex-1 px-3 py-2 rounded-xl text-xs font-medium bg-white/5 border border-white/10 hover:bg-white/10 disabled:opacity-40 flex items-center justify-center gap-1.5"><Trash2 className="w-3.5 h-3.5" /> {t("Effacer pinceau")}</button>
            <button onClick={undo} disabled={!history.length} title={t("Annuler la dernière suppression")} className="px-3 py-2 rounded-xl text-xs font-medium bg-white/5 border border-white/10 hover:bg-white/10 disabled:opacity-40 flex items-center justify-center gap-1.5"><Undo2 className="w-3.5 h-3.5" /></button>
          </div>

          {/* Output dir */}
          <div>
            <label className="text-[11px] font-semibold text-gray-400 uppercase tracking-wider">{t("Dossier de sortie")}</label>
            <div className="flex gap-2 mt-1.5">
              <input className="bg-white/5 border border-white/10 rounded-xl px-3 py-2 text-sm text-gray-200 outline-none hover:bg-white/10 focus:border-rose-500/50 transition-all w-full" value={outputDir} onChange={e => setOutputDir(e.target.value)} placeholder={t("Dossier…")} />
          ...
          <button onClick={run} disabled={!img || processing || !hasMask}
            ...
            {processing ? <Loader2 className="w-4 h-4 animate-spin" /> : <Wand2 className="w-4 h-4" />}{processing ? (stage || t('Traitement…')) : (prompt.trim() ? t('Générer dans la sélection') : t('Effacer la sélection'))}
          </button>
          {img && (
            <div className="flex gap-2">
              <button onClick={() => electron.showItemInFolder?.(img.path)} className="flex-1 px-3 py-2 rounded-xl text-xs font-medium bg-white/5 border border-white/10 hover:bg-white/10 flex items-center justify-center gap-1.5"><Download className="w-3.5 h-3.5" /> {t("Dossier")}</button>
              <button onClick={() => electron.openFile?.(img.path)} className="flex-1 px-3 py-2 rounded-xl text-xs font-medium bg-white/5 border border-white/10 hover:bg-white/10 flex items-center justify-center gap-1.5"><Maximize2 className="w-3.5 h-3.5" /> {t("Ouvrir")}</button>
            </div>
          )}
        </div>
            ...
            <div className="relative max-w-full max-h-full inline-block" style={{ lineHeight: 0 }}
              ...
              {processing && (
                <div className="absolute inset-0 rounded-xl bg-black/55 backdrop-blur-[2px] flex flex-col items-center justify-center gap-3">
                  ...
                  <span className="text-sm text-gray-200">{stage || t('Traitement IA…')}</span>
                  {!prompt.trim()
                    ? <span className="text-[11px] text-gray-500">{t("Mode effacer (LaMa) : la 1ʳᵉ fois, le moteur se télécharge (~200 Mo)")}</span>
                    : <span className="text-[11px] text-gray-500 text-center">{t("Mode générer : Stable Diffusion 100% local · accéléré")} <span className="text-gray-400">GPU</span> {t("(DirectML, repli CPU auto) · 1ʳᵉ fois ~2,1 Go")}</span>}
                </div>
              )}
            </div>
          )}
