...
export default function HandBrake() {
  ...
  useEffect(() => { if (!electron?.hbQueueSave) return; const tm = setTimeout(() => electron.hbQueueSave(jobs), 600); return () => clearTimeout(tm); }, [jobs]);
  useEffect(() => {
    ...
    electron.onHbComplete?.((d: any) => { setJobs(prev => prev.map(j => j.id === d.id ? { ...j, status: 'done', percent: 100, outputPath: d.outputPath, stage: t('Terminé') } : j)); setTimeout(() => runNext(), 400); });
  ...
  const startJob = (job: Job) => { setJobs(prev => prev.map(j => j.id === job.id ? { ...j, status: 'running', percent: 0, stage: t('Démarrage…'), error: undefined } : j)); electron.hbStart?.({ id: job.id, inputPath: job.inputPath, ...job.settings, whenDone: 'none' }); };
  ...
  if (detect && !detect.installed) {
    return (
      <div className="h-full flex flex-col items-center justify-center gap-4 text-center px-8">
        ...
        <p className="text-gray-400 max-w-md text-sm">{t("Le moteur HandBrake (open-source) sera téléchargé et installé automatiquement (~26 Mo). Tous les membres y ont accès, aucune licence requise.")}</p>
        ...
        <button onClick={install} disabled={installing} className="px-5 py-2.5 rounded-xl bg-orange-500/20 border border-orange-500/40 text-orange-200 hover:bg-orange-500/30 transition-all text-sm font-semibold flex items-center gap-2 disabled:opacity-50">
          {installing ? <Loader2 className="w-4 h-4 animate-spin" /> : <Download className="w-4 h-4" />}{installing ? t('Installation…') : t('Installer HandBrake')}
        </button>
      </div>
    );
  }
    ...
    <div className="h-full flex flex-col text-gray-300 overflow-hidden">
      <div className="flex items-center justify-between px-6 py-3 border-b border-white/5">
        <div className="flex items-center gap-3">
          ...
          <div><h2 className="text-lg font-bold text-white">HandBrake</h2><p className="text-[11px] text-gray-500">{t("Compression vidéo · moteur HandBrakeCLI officiel")}{detect?.presets?.flat?.length ? ` · ${t("{n} préréglages", { n: detect.presets.flat.length })}` : ''}</p></div>
        </div>
      </div>
      ...
      <div className="flex-1 overflow-hidden flex">
        ...
        <div className="w-[330px] shrink-0 border-r border-white/5 flex flex-col">
          ...
          <div className="flex-1 overflow-y-auto px-3 pb-3 space-y-2">
            ...
            {jobs.map(job => (
              <button key={job.id} onClick={() => setSelectedId(job.id)} className={`w-full text-left rounded-xl border p-2 flex gap-2.5 transition-all ${selectedId === job.id ? 'border-orange-500/50 bg-orange-500/10' : 'border-white/8 bg-white/[0.03] hover:bg-white/[0.06]'}`}>
                ...
                <div className="flex-1 min-w-0"><p className="text-xs text-gray-200 truncate font-medium">{job.name}</p><p className="text-[10px] text-gray-500 truncate">{job.meta ? `${job.meta.width}×${job.meta.height} · ${fmtDur(job.meta.duration)} · ${fmtSize(job.meta.size)}` : t('Analyse…')}</p>
                  ...
                  {job.status === 'done' && <span className="text-[10px] text-green-400">{t("✓ Terminé")}</span>}{job.status === 'error' && <span className="text-[10px] text-red-400">{t("✕ Erreur")}</span>}
                </div>
          ...
          <div className="p-3 border-t border-white/5"><button onClick={startAll} disabled={!jobs.length} className="w-full px-3 py-2.5 rounded-xl text-sm font-bold flex items-center justify-center gap-2 transition-all disabled:opacity-40" style={{ background: 'linear-gradient(135deg,rgba(249,115,22,0.75),rgba(239,68,68,0.75))', border: '1px solid rgba(255,255,255,0.2)', color: 'white', boxShadow: '0 4px 20px rgba(249,115,22,0.3)' }}><Play className="w-4 h-4 fill-current" /> {t("Encoder la file")}</button></div>
        </div>

        {/* Settings */}
        <div className="flex-1 overflow-y-auto">
          {!selected ? <div className="h-full flex items-center justify-center text-gray-600 text-sm">{t("Sélectionnez un fichier pour configurer l'encodage")}</div> : (
            <div className="p-5 space-y-4 max-w-3xl">
              <div className="flex items-center gap-2 flex-wrap">
                {BUILTIN.map(p => <button key={p.name} onClick={() => applyPreset(p.patch as any)} className="px-3 py-1.5 rounded-xl text-xs font-medium bg-white/5 border border-white/10 hover:bg-white/10 hover:border-orange-500/30 transition-all flex items-center gap-1.5"><span>{p.icon}</span>{t(p.name)}</button>)}
                ...
                <button onClick={savePreset} className="px-3 py-1.5 rounded-xl text-xs font-medium bg-white/5 border border-white/10 hover:bg-white/10 flex items-center gap-1.5"><Save className="w-3 h-3" /> {t("Enregistrer")}</button>
              </div>

              <Section title={t("Préréglage & Format")} icon={<Film className="w-3.5 h-3.5 text-orange-400" />}>
                <div className="grid grid-cols-2 gap-3">
                  <div><label className={LABEL}>{t("Préréglage HandBrake (base)")}</label>
                    <GlassSelect className="mt-1 w-full" value={selected.settings.preset} onChange={v => patch({ preset: v })} ariaLabel={t("Préréglage HandBrake")}
                      options={[{ value: '', label: t('Aucun (réglages manuels)') }, ...Object.entries(presetGroups).flatMap(([g, names]: any) => (names as string[]).map((n: string) => ({ value: n, label: n, group: g })))]} />
                  </div>
                  <div><label className={LABEL}>{t("Conteneur")}</label><GlassSelect className="mt-1 w-full" value={selected.settings.container} onChange={v => patch({ container: v })} ariaLabel={t("Conteneur")} options={CONTAINERS.map(c => ({ value: c, label: c }))} /></div>
                </div>
                {selected.settings.container === 'mp4' && <label className="flex items-center gap-2 text-xs text-gray-300 cursor-pointer"><Toggle on={selected.settings.webOptimize} onClick={() => patch({ webOptimize: !selected.settings.webOptimize })} /> {t("Optimisé web (lecture progressive)")}</label>}
              </Section>

              <Section title={t("Vidéo")} icon={<Sliders className="w-3.5 h-3.5 text-orange-400" />}>
                <div className="grid grid-cols-2 gap-3">
                  <div><label className={LABEL}>{t("Encodeur")}</label><GlassSelect className="mt-1 w-full" value={selected.settings.encoder} onChange={v => patch({ encoder: v, encoderPreset: 'medium' })} ariaLabel={t("Encodeur")} options={encoders.map((e: any) => ({ value: e.v, label: e.l }))} /></div>
                  <div><label className={LABEL}>{t("Vitesse d'encodage")}</label><GlassSelect className="mt-1 w-full" value={selected.settings.encoderPreset} onChange={v => patch({ encoderPreset: v })} ariaLabel={t("Vitesse d'encodage")} options={speedPresets.map((p: string) => ({ value: p, label: p }))} /></div>
                </div>
                <div className="flex gap-2">
                  {(['quality', 'bitrate'] as const).map(m => <button key={m} onClick={() => patch({ rateMode: m })} className={`px-4 py-1.5 rounded-lg text-xs font-semibold transition-all ${selected.settings.rateMode === m ? 'bg-orange-500/30 text-orange-100 border border-orange-500/50' : 'bg-white/5 text-gray-400 border border-white/10'}`}>{m === 'quality' ? t('Qualité constante (RF)') : t('Débit moyen')}</button>)}
                </div>
                ...
                <div className="grid grid-cols-3 gap-3">
                  ...
                  <div><label className={LABEL}>{t("Rotation")}</label><GlassSelect className="mt-1 w-full" value={String(selected.settings.rotate)} onChange={v => patch({ rotate: Number(v) })} ariaLabel={t("Rotation")} options={ROTATIONS.map(r => ({ value: String(r.v), label: r.l }))} /></div>
                </div>
              </Section>

              <Section title={t("Filtres")} icon={<Sliders className="w-3.5 h-3.5 text-orange-400" />}>
                <div className="grid grid-cols-2 gap-3">
                  <div><label className={LABEL}>{t("Débruitage (NLMeans)")}</label><GlassSelect className="mt-1 w-full" value={selected.settings.denoise} onChange={v => patch({ denoise: v })} ariaLabel={t("Débruitage")} options={denoiseOpts.map((d: string) => ({ value: d, label: d === 'off' ? t('Aucun') : d }))} /></div>
                  <div><label className={LABEL}>{t("Netteté (Lapsharp)")}</label><GlassSelect className="mt-1 w-full" value={selected.settings.sharpen} onChange={v => patch({ sharpen: v })} ariaLabel={t("Netteté")} options={sharpenOpts.map((d: string) => ({ value: d, label: d === 'off' ? t('Aucun') : d }))} /></div>
                </div>
                <div className="grid grid-cols-2 gap-x-6 gap-y-2">
                  <label className="flex items-center gap-2 text-xs text-gray-300 cursor-pointer"><Toggle on={selected.settings.deinterlace} onClick={() => patch({ deinterlace: !selected.settings.deinterlace })} /> {t("Désentrelacement (decomb)")}</label>
                  <label className="flex items-center gap-2 text-xs text-gray-300 cursor-pointer"><Toggle on={selected.settings.deblock} onClick={() => patch({ deblock: !selected.settings.deblock })} /> {t("Deblock")}</label>
                  <label className="flex items-center gap-2 text-xs text-gray-300 cursor-pointer"><Toggle on={selected.settings.grayscale} onClick={() => patch({ grayscale: !selected.settings.grayscale })} /> {t("Noir & blanc")}</label>
                </div>
              </Section>

              <Section title={t("Audio & Sortie")} icon={<Download className="w-3.5 h-3.5 text-orange-400" />}>
                <div className="grid grid-cols-2 gap-3">
                  <div><label className={LABEL}>{t("Audio")}</label><GlassSelect className="mt-1 w-full" value={selected.settings.audioMode} onChange={v => patch({ audioMode: v })} ariaLabel={t("Audio")}
                    options={[{ value: 'copy', label: t('Copier (sans ré-encodage)') }, { value: 'aac', label: selected.settings.container === 'webm' ? 'Opus' : 'AAC' }, { value: 'none', label: t('Aucun') }]} /></div>
                  {selected.settings.audioMode === 'aac' && <div><label className={LABEL}>{t("Débit audio (kbit/s)")}</label><input type="number" className={INPUT + ' mt-1'} value={selected.settings.audioBitrate} onChange={e => patch({ audioBitrate: Number(e.target.value) })} /></div>}
                </div>
                {selected.settings.container !== 'webm' && <label className="flex items-center gap-2 text-xs text-gray-300 cursor-pointer"><Toggle on={selected.settings.subtitles} onClick={() => patch({ subtitles: !selected.settings.subtitles })} /> {t("Conserver les sous-titres")}</label>}
                <div className="flex gap-2 items-end"><div className="flex-1"><label className={LABEL}>{t("Dossier de sortie")}</label><input className={INPUT + ' mt-1'} value={selected.settings.outputDir} onChange={e => patch({ outputDir: e.target.value })} placeholder={t("Dossier de destination…")} /></div><button onClick={browseOutput} className="px-3 py-2 rounded-xl text-sm bg-white/5 border border-white/10 hover:bg-white/10"><FolderOpen className="w-4 h-4" /></button></div>
              </Section>

              <div className="flex items-center gap-3 pt-1">
                {selected.status === 'running' ? <button onClick={() => cancelJob(selected)} className="flex-1 px-4 py-3 rounded-xl font-bold text-sm bg-red-500/20 border border-red-500/40 text-red-200 hover:bg-red-500/30 flex items-center justify-center gap-2"><Square className="w-4 h-4" /> {t("Arrêter")} ({selected.percent}% · {selected.stage})</button>
                  : selected.status === 'done' ? <><button onClick={() => selected.outputPath && electron.showItemInFolder?.(selected.outputPath)} className="flex-1 px-4 py-3 rounded-xl font-bold text-sm bg-green-500/20 border border-green-500/40 text-green-200 hover:bg-green-500/30 flex items-center justify-center gap-2"><CheckCircle2 className="w-4 h-4" /> {t("Terminé — Ouvrir")}</button><button onClick={() => startJob(selected)} className="px-4 py-3 rounded-xl bg-white/5 border border-white/10 hover:bg-white/10"><RotateCcw className="w-4 h-4" /></button></>
                    : <button onClick={() => startJob(selected)} className="flex-1 px-4 py-3 rounded-xl font-bold text-sm flex items-center justify-center gap-2 transition-all" style={{ background: 'linear-gradient(135deg,rgba(249,115,22,0.75),rgba(239,68,68,0.75))', border: '1px solid rgba(255,255,255,0.2)', color: 'white', boxShadow: '0 4px 24px rgba(249,115,22,0.4)' }}><Play className="w-4 h-4 fill-current" /> {t("Encoder cette vidéo")}</button>}
                <button onClick={applyToAll} title={t("Appliquer à toute la file")} className="px-4 py-3 rounded-xl text-sm bg-white/5 border border-white/10 hover:bg-white/10 flex items-center gap-2"><Layers className="w-4 h-4" /> {t("À tous")}</button>
              </div>
