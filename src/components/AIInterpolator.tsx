...
export default function AIInterpolator() {
  ...
  const [tabConfig, setTabConfig] = useState<{ id: string; label: string; visible: boolean }[]>(() => {
    ...
    return defaultTabs.map(t => ({ ...t, visible: true }));
  });
  ...
  const [logs, setLogs] = useState<LogEntry[]>([
    { type: 'info', text: 'Bienvenue dans Orbit Interpolator !', ts: new Date().toLocaleTimeString() },
    { type: 'info', text: 'Sélectionnez une vidéo et appuyez sur "Interpoler !" pour commencer.', ts: new Date().toLocaleTimeString() },
  ]);
  ...
  useEffect(() => {
    ...
    electronAPI.onAIInterpolateProgress?.((data: any) => {
      ...
      if (msg.includes('Extraction')) {
        setProgress(10);
        setProgressText('Extraction des frames...');
      } else if (msg.includes('Interpolation IA')) {
        setProgress(20);
        setProgressText('Interpolation IA en cours...');
      } else if (msg.includes('Recomposition')) {
        setProgress(80);
        setProgressText('Recomposition vidéo...');
      } else if (msg.includes('Téléchargement')) {
        setProgress(5);
        setProgressText('Téléchargement RIFE-NCNN...');
      }
      ...
      if (frameMatch) {
        ...
        if (total > 0) {
          ...
          setProgressText(`Frame ${current}/${total}`);
        }
      }
    });
    ...
    electronAPI.onAIInterpolateComplete?.((data: any) => {
      ...
      addLog('success', '✅ Interpolation terminée avec succès !');
      if (data?.filePath) {
        addLog('success', `Fichier : ${data.filePath.split('\\').pop()}`);
      }
    });
    ...
    electronAPI.onAIInterpolateError?.((data: any) => {
      ...
      setProgress(0);
      setProgressText('');
      addLog('error', data?.error || 'Une erreur est survenue.');
    });
  }, [electronAPI]);
  ...
  const handleBrowseVideo = async () => {
    ...
    if (file) {
      setInputVideo(file);
      addLog('info', `Vidéo sélectionnée : ${file.split('\\').pop()}`);
      // Get FPS
  ...
  const handleBrowseOutput = async () => {
    ...
    if (dir) {
      setOutputDir(dir);
      addLog('info', `Dossier de sortie : ${dir}`);
    }
  };

  const handleInterpolate = () => {
    if (!inputVideo) {
      addLog('error', 'Veuillez sélectionner une vidéo source.');
      return;
    }
    if (!outputDir) {
      addLog('error', 'Veuillez sélectionner un dossier de sortie.');
      return;
    }
    setStatus('running');
    setProgress(0);
    ...
    setLogs([]);
    addLog('info', `Démarrage de l'interpolation (${engine})...`);
    addLog('info', `Multiplicateur : ${multiplier} | Format : ${outFormat} | Codec : ${codec}`);
    addLog('info', `Vidéo : ${inputVideo.split('\\').pop()}`);
    ...
    if (electronAPI?.aiInterpolate) {
      ...
      addLog('error', 'API Electron non disponible.');
      setStatus('error');
    }
  };
        ...
        {activeTab === 'interpolation' && (
          <div className="flex-1 overflow-y-auto flex flex-col gap-0">
            ...
            <div className="p-6 space-y-3">
              ...
              {/* Input Video */}
              <div className="glass-panel rounded-xl p-3 flex items-center gap-4">
                <div className="w-44 shrink-0"><label className={LABEL_CLS}><FolderOpen className="w-3 h-3" /> Vidéo Source</label></div>
                <input type="text" value={inputVideo} onChange={e => setInputVideo(e.target.value)}
                  placeholder="Cliquez Parcourir ou glissez un fichier..."
              ...
              {/* Output Directory */}
              <div className="glass-panel rounded-xl p-3 flex items-center gap-4">
                <div className="w-44 shrink-0"><label className={LABEL_CLS}><FolderOpen className="w-3 h-3" /> Dossier Sortie</label></div>
                <input type="text" value={outputDir} onChange={e => setOutputDir(e.target.value)}
                  placeholder="Dossier de destination..." className={INPUT_CLS} disabled={status === 'running'} />
              ...
              <div className="flex gap-3">
                <div className="glass-panel rounded-xl p-3 flex items-center gap-3 flex-1">
                  <div className="w-32 shrink-0"><label className={LABEL_CLS}><Zap className="w-3 h-3" /> Multiplicateur</label></div>
              ...
              <div className="flex items-center gap-3 justify-between mt-2">
                ...
                >
                  ...
                  {status === 'running' ? (
                    <span className="relative flex items-center gap-2"><Loader2 className="w-4 h-4 animate-spin" />En cours...</span>
                  ) : (
                    <span className="relative flex items-center gap-2"><Play className="w-4 h-4 fill-current" />Interpoler !</span>
                  )}
                </motion.button>
              </div>
            </div>
            ...
            <div className="flex-1 mx-6 mb-6 glass-panel rounded-xl p-4 flex flex-col min-h-[150px]">
              <div className="flex items-center gap-2 mb-2">
                ...
                <p className="text-[10px] text-gray-500 uppercase font-semibold">Console</p>
                <span className="text-[10px] text-gray-600 ml-auto">{logs.length} entrées</span>
              </div>
        ...
        {activeTab === 'quick' && (
          <div className="p-6 space-y-4">
            <div className="glass-panel rounded-xl p-6">
              <h3 className="font-bold text-white mb-4">Préréglages Rapides</h3>
              <div className="grid grid-cols-3 gap-3">
                {[
                  { name: 'Anime 60FPS', desc: 'RIFE x2 - Optimisé anime', icon: '🎌', mult: 'x2', fmt: 'MP4' },
                  { name: 'Film Fluide', desc: 'RIFE x2 - Qualité lossless', icon: '🎬', mult: 'x2', fmt: 'MKV' },
                  { name: 'Slow Motion', desc: 'RIFE x4 - Ralenti x0.5', icon: '🐌', mult: 'x4', fmt: 'MP4' },
                  { name: 'Super Fluide 120', desc: 'RIFE x4 - 120FPS', icon: '⚡', mult: 'x4', fmt: 'MP4' },
                  { name: 'Discord GIF', desc: 'RIFE x2 → GIF', icon: '💬', mult: 'x2', fmt: 'GIF' },
                  { name: 'Max FPS', desc: 'RIFE x8 - 240FPS', icon: '🚀', mult: 'x8', fmt: 'MP4' },
                ].map(preset => (
                  <button
                    key={preset.name}
                    onClick={() => {
        ...
        {activeTab === 'about' && (
          <div className="p-6 flex flex-col items-center justify-center h-full gap-4">
            ...
            <p className="text-gray-500 text-sm text-center max-w-md">
              Interpolation vidéo par IA utilisant RIFE-NCNN (Real-time Intermediate Flow Estimation).
              Compatible GPU Vulkan (NVIDIA, AMD, Intel).
            </p>
            <div className="grid grid-cols-2 gap-3 mt-4 w-full max-w-md">
              {[
                { label: 'Moteur', value: 'RIFE-NCNN Vulkan' },
                { label: 'Accélération', value: 'GPU (Vulkan)' },
                { label: 'Modèle', value: 'RIFE v4.26' },
                { label: 'Formats', value: 'MP4, MKV, MOV, GIF, WEBM' },
              ].map(item => (
