...
export default function MattingStudio() {
  ...
  useEffect(() => {
    ...
    electron.onMattingComplete?.((d: any) => { setJobs(prev => prev.map(j => j.id === d.id ? { ...j, status: 'done', percent: 100, outputPath: d.outputPath, stage: t('Terminé') } : j)); setTimeout(() => runNext(), 400); });
  ...
  const startJob = (job: Job) => { setJobs(prev => prev.map(j => j.id === job.id ? { ...j, status: 'running', percent: 0, stage: t('Démarrage…'), error: undefined } : j)); electron.mattingStart?.(toSpec(job)); };
  ...
  const installModel = async (key: string) => { setInstalling(true); const r = await electron.mattingInstall?.(key).catch((e: any) => ({ ok: false, error: String(e) })); setInstalling(false); if (r?.ok) { showToast('info', t('Modèle installé ✓')); setDetect(await electron.mattingDetect?.()); } else showToast('error', r?.error || t('Échec')); };
  const genPreview = async () => { if (!selected) return; setPreviewing(true); setPreviewUrl(''); const r = await electron.mattingPreview?.(toSpec(selected, { start: 0, duration: 3 })).catch((e: any) => ({ error: String(e) })); setPreviewing(false); if (r?.outputPath) setPreviewUrl(mediaUrl(r.outputPath)); else showToast('error', r?.error || t('Aperçu impossible.')); };

  if (detect && detect.ready === false) {
    return <div className="h-full flex flex-col items-center justify-center gap-3 text-center px-8"><div className="w-20 h-20 rounded-2xl bg-gradient-to-br from-teal-500/20 to-cyan-500/20 flex items-center justify-center border border-white/10"><Scissors className="w-9 h-9 text-teal-400" /></div><h2 className="text-2xl font-bold text-white">{t("Moteur IA indisponible")}</h2><p className="text-gray-400 max-w-md text-sm">{t("onnxruntime n'a pas pu se charger. Réinstallez l'application.")}</p><p className="text-[11px] text-gray-600">{detect.err}</p></div>;
  }
          ...
          <div className="flex-1 overflow-y-auto px-3 pb-3 space-y-2">
            ...
            {jobs.map(job => (
              <button key={job.id} onClick={() => { setSelectedId(job.id); setPreviewUrl(''); }} className={`w-full text-left rounded-xl border p-2 flex gap-2.5 transition-all ${selectedId === job.id ? 'border-teal-500/50 bg-teal-500/10' : 'border-white/8 bg-white/[0.03] hover:bg-white/[0.06]'}`}>
                ...
                <div className="flex-1 min-w-0"><p className="text-xs text-gray-200 truncate font-medium">{job.name}</p><p className="text-[10px] text-gray-500 truncate">{job.meta ? `${job.meta.width}×${job.meta.height} · ${fmtDur(job.meta.duration)}` : t('Analyse…')}</p>{job.status === 'running' && <div className="mt-1 h-1.5 bg-white/10 rounded-full overflow-hidden"><div className="h-full bg-gradient-to-r from-teal-500 to-cyan-500" style={{ width: `${job.percent}%` }} /></div>}{job.status === 'done' && <span className="text-[10px] text-green-400">{t("✓ Terminé")}</span>}{job.status === 'error' && <span className="text-[10px] text-red-400">{t("✕ Erreur")}</span>}</div>
            ...
            <div className="p-5 grid grid-cols-2 gap-5 max-w-5xl">
              ...
              <div className="space-y-4">
                <div className="rounded-2xl border border-white/8 bg-white/[0.02] p-4 space-y-3">
                  <div className="flex items-center gap-2"><Wand2 className="w-3.5 h-3.5 text-teal-400" /><span className="text-sm font-semibold text-gray-200">{t("Modèle & qualité")}</span></div>
                  <div className="grid grid-cols-2 gap-3">
                    <div><label className={LABEL}>{t("Modèle IA")}</label>
                      <GlassSelect className="mt-1 w-full" value={s.model} onChange={v => patch({ model: v })} ariaLabel={t("Modèle IA")}
                        options={(detect?.models || [{ key: 'mobilenetv3', label: 'MobileNetV3 (rapide)' }, { key: 'resnet50', label: 'ResNet50 (qualité)' }]).map((m: any) => ({ value: m.key, label: `${m.label}${m.installed === false ? t(' — à télécharger') : ''}` }))} />
                    </div>
                    <div><label className={LABEL}>{t("Vitesse / qualité")}</label>
                      <GlassSelect className="mt-1 w-full" value={s.quality} onChange={v => patch({ quality: v })} ariaLabel={t("Vitesse / qualité")}
                        options={[{ value: 'fast', label: t('Rapide (512p)') }, { value: 'balanced', label: t('Équilibré (960p)') }, { value: 'max', label: t('Max (source)') }]} />
                    </div>
                  </div>
                  {detect?.models?.find((m: any) => m.key === s.model && !m.installed) && (
                    <button onClick={() => installModel(s.model)} disabled={installing} className="w-full text-xs py-2 rounded-lg bg-teal-500/15 border border-teal-500/30 text-teal-200 flex items-center justify-center gap-1.5 disabled:opacity-50">{installing ? <Loader2 className="w-4 h-4 animate-spin" /> : <Download className="w-4 h-4" />}{installing ? (installLog || t('Téléchargement…')) : t('Télécharger ce modèle')}</button>
                  )}
                </div>

                <div className="rounded-2xl border border-white/8 bg-white/[0.02] p-4 space-y-3">
                  <span className="text-sm font-semibold text-gray-200">{t("Arrière-plan")}</span>
                  <div className="flex gap-1.5 flex-wrap">
                    {MODES.map(m => <button key={m.v} onClick={() => patch({ mode: m.v })} className={`px-3 py-1.5 rounded-lg text-xs font-semibold transition-all flex items-center gap-1.5 ${s.mode === m.v ? 'bg-teal-500/30 text-teal-100 border border-teal-500/50' : 'bg-white/5 text-gray-400 border border-white/10 hover:bg-white/10'}`}><span>{m.emoji}</span>{t(m.l)}</button>)}
                  </div>
                  {s.mode === 'transparent' && (
                    <div><label className={LABEL}>{t("Format (avec alpha)")}</label>
                      <GlassSelect className="mt-1 w-full" value={s.transparentFormat} onChange={v => patch({ transparentFormat: v })} ariaLabel={t("Format (avec alpha)")}
                        options={[{ value: 'webm', label: t('WebM (VP9 · web/montage)') }, { value: 'prores', label: t('ProRes 4444 (.mov · pro)') }, { value: 'png', label: t('Séquence PNG (VFX)') }]} />
                      <p className="text-[10px] text-gray-500 mt-1">{t("Le fond devient transparent — idéal incrustation AE / Premiere.")}</p>
                    </div>
                  )}
                  {s.mode === 'color' && <div><label className={LABEL}>{t("Couleur de fond")}</label><div className="flex items-center gap-2 mt-1"><input type="color" value={s.color} onChange={e => patch({ color: e.target.value })} className="w-10 h-9 rounded-lg bg-transparent border border-white/10 cursor-pointer" /><input className={INPUT} value={s.color} onChange={e => patch({ color: e.target.value })} /></div></div>}
                  {s.mode === 'blur' && <Slider label={t("Intensité du flou")} value={s.blurStrength} min={2} max={60} onChange={(v: number) => patch({ blurStrength: v })} />}
                  {s.mode === 'image' && <div><label className={LABEL}>{t("Image de fond")}</label><div className="flex gap-2 mt-1"><input className={INPUT} value={s.bgImage} onChange={e => patch({ bgImage: e.target.value })} placeholder={t("Choisir une image…")} /><button onClick={browseBg} className="px-3 py-2 rounded-xl bg-white/5 border border-white/10 hover:bg-white/10"><FolderOpen className="w-4 h-4" /></button></div></div>}
                </div>

                <div className="rounded-2xl border border-white/8 bg-white/[0.02] p-4 space-y-3">
                  <span className="text-sm font-semibold text-gray-200">{t("Affinage des bords")}</span>
                  <Slider label={t("Adoucissement (anti-crénelage)")} value={s.feather} min={0} max={4} step={0.1} onChange={(v: number) => patch({ feather: v })} suffix="px" />
                  <Slider label={t("Resserrer ◀ / Élargir ▶ le masque")} value={s.choke} min={-40} max={40} onChange={(v: number) => patch({ choke: v })} />
                  <p className="text-[10px] text-gray-500">{t("Resserrer enlève le liseré de fond autour du sujet ; adoucir lisse les contours.")}</p>
                </div>

                <div className="rounded-2xl border border-white/8 bg-white/[0.02] p-4 space-y-3">
                  <span className="text-sm font-semibold text-gray-200">{t("Sortie")}</span>
                  <div className="flex gap-2 items-end"><div className="flex-1"><label className={LABEL}>{t("Dossier")}</label><input className={INPUT + ' mt-1'} value={s.outputDir} onChange={e => patch({ outputDir: e.target.value })} placeholder={t("Dossier de destination…")} /></div><button onClick={browseOutput} className="px-3 py-2 rounded-xl bg-white/5 border border-white/10 hover:bg-white/10"><FolderOpen className="w-4 h-4" /></button></div>
                </div>
                ...
                <div className="rounded-2xl border border-white/10 overflow-hidden aspect-video flex items-center justify-center" style={CHECKER}>
                  {previewUrl ? <video src={previewUrl} className="max-h-full max-w-full" autoPlay loop muted controls />
                    : selected.thumb ? <img src={selected.thumb} alt="" className="max-h-full max-w-full opacity-60" />
                      : <p className="text-xs text-gray-500">{t("Générez un aperçu pour visualiser le détourage")}</p>}
                </div>
                <p className="text-[10px] text-gray-500 text-center">{t("Damier = zones transparentes. L'aperçu applique vos réglages sur 3 s.")}</p>
              </div>
            </div>
          )}
        </div>
