...
<p align="center">
  <a href="https://github.com/ThomasDM40-dev/orbit-releases/releases/latest/download/Orbit-Installer.exe">
    <img src="https://img.shields.io/badge/⬇️_Télécharger-Orbit_Installer.exe-ff2a6d?style=for-the-badge" alt="Download"/>
  </a>
</p>
...
> Ce dépôt héberge les **releases** d'Orbit (installeur + mises à jour automatiques). Les binaires bundlés (yt-dlp, FFmpeg, Real-ESRGAN, RIFE, HandBrakeCLI, Whisper) sont téléchargés automatiquement au premier usage. Le code source n'est pas distribué.
...
- **After Effects** :
  - **Compatibilité de version** — rends un preset `.ffx` (ou projet `.aep`) compatible avec un AE **plus ancien ou plus récent**, en un clic, **instantanément** et **localement** (réécriture non-destructive du champ de version ; cibles lues depuis tes AE installés). Marche par lot.
  - **Analyse** des presets (type RIFX + **plugins tiers requis** : Magic Bullet, Sapphire, Red Giant…)
  - **Pont AE local** : Orbit pilote ton After Effects installé (ExtendScript) pour **mettre à niveau un `.aep`** vers ta version et **appliquer un preset `.ffx`** en projet
...
- **v0.34.9** — correctif **définitif** des cercles de chargement (couche GPU garantie : ne figent plus, même pendant un envoi Drive)
